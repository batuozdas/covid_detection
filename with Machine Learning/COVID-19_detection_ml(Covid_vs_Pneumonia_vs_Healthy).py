...
for model_name in model_params:
    ...
    label_names = ['Covid_Precision', 'Covid_Recall', 'Covid_F1-score', 'Pneumonia_Precision', 'Pneumonia_Recall',
                   'Pneumonia_F1-score',
                   'Healthy_Precision', 'Healthy_Recall', 'Healthy_F1-score']
    ...
    if model_name == 'Support Vector Machines':
        ...
        for i in (df_results_model['param_C']):
            ...
            y_pred = model.predict(X_test)
            cr = classification_report(y_test, y_pred, output_dict=True, zero_division=0)
            cr_s[str(m)] = np.array(list(map(lambda k: [cr[str(k)]['precision'], cr[str(k)]['recall'], cr[str(k)]['f1-score']],range(0, 3)))).flatten()
            m += 1
...
df_final_results2 = df_final_results.loc[:,'Model Adı':'Healthy_F1-score'].drop(['param_criterion','param_n_estimators'],axis='columns')
...
df_final_results2.to_excel('Covid_vs_Pneumonia_vs_Healthy.xlsx')

best_model_df = df_final_results2[df_final_results2['Test Doğruluk'] == np.max(df_final_results2['Test Doğruluk'])]

best_model = svm.SVC(C=30,kernel='linear',gamma='auto')
best_model.fit(X_train,y_train)
y_pred_best = best_model.predict(X_test)

# Confusion Matrix
cm = confusion_matrix(y_test,y_pred_best)
df_cm = pd.DataFrame(cm,index=['Covid-19','Zatürre','Sağlıklı'],columns=['Covid-19','Zatürre','Sağlıklı'])
...
roc_labels = [['Covid-19_test','Covid-19_pred'],['Zatürre_test','Zatürre_pred'],['Sağlıklı_test','Sağlıklı_pred']]
iteration = 0
y_test = np.array(y_test)
titles = []
for i,j in roc_labels:
    globals()[i] = np.where(y_test == iteration,0,1)
    globals()[j] = np.where(y_pred_best == iteration,0,1)
    fpr, tpr, _ = roc_curve(globals()[i], globals()[j])
    titles.append(i.split('_')[0])
    plt.plot(fpr,tpr)
    iteration += 1
plt.xlim([0, 1])
plt.ylim([0, 1])
